...
fn aqw_script_rearm_fix_disabled() -> bool {
    static DISABLED: OnceLock<bool> = OnceLock::new();
    *DISABLED.get_or_init(|| {
        crate::display_object::aqw_env_flag("RUFFLE_AQW_NO_SCRIPT_REARM_FIX", false)
    })
}
...
impl<'gc> MovieClip<'gc> {
    ...
    fn check_has_pending_script(self, context: &mut UpdateContext<'gc>) {
        let frame = self.0.current_frame.get();
        let mut has_pending_script = self.has_frame_script(frame);
        if has_pending_script
            && !aqw_script_rearm_fix_disabled()
            && self.last_queued_script_frame() == Some(frame)
        {
